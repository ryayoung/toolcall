# File generated from its async equivalent, tests/test_oai_aio.py
...
def test_func_tool_handler():
    def main():
        ...
        with pytest.raises(TypeError):
            group.run_tool_call(make_call("Tool", '{"x": 2}'), None)
        with pytest.raises(TypeError):
            group.run_tool_call(make_call("Tool", '{"x": 3}'), None)
...
def test_custom_tool_handler():
    def main():
        ...
        with pytest.raises(TypeError):
            group.run_tool_call(make_call("Tool", "2"), None)
        with pytest.raises(TypeError):
            group.run_tool_call(make_call("Tool", "3"), None)
        with pytest.raises(ValueError):
            group.run_tool_call(make_call("ExtraFieldTool", "0"), None)
