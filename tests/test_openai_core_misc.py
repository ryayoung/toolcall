...
def test_misc():
    ...
    MyEmptyTool.model_tool_run_tool_call(invalid_tool_call, None)
    ...
    with pytest.raises(NotImplementedError):
        ...
        MyEmptyTool.model_tool_run_tool_call(tool_call, None)

    class BadHandlerTool(LLMFunctionTool):
        def model_tool_handler(self, context: None) -> tuple:
    ...
    with pytest.raises(TypeError):
        ...
        BadHandlerTool.model_tool_run_tool_call(tool_call, None)
        BadHandlerTool2.model_tool_run_tool_call(tool_call, None)

    group = LLMFunctionToolGroup[None, None].from_list([MyEmptyTool, BadHandlerTool])
    group.tool_names
    group.tool_classes
    ...
    group.run_tool_call(make_tc_chat("NotATool", "{}"), None)
